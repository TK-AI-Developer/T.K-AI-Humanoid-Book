...
<launch>
  ...
  <!-- Launch the perception flow processor -->
  <node pkg="digital_twin_gazebo" exec="perception_flow_processor.py" name="perception_flow_processor">
    <param name="use_sim_time" value="$(var use_sim_time)"/>
    <param name="complexity_level" value="medium"/>
    <param name="enable_noise_models" value="true"/>
  </node>
</launch>