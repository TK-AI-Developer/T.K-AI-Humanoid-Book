...
<launch>
  ...
  <!-- Robot State Publisher for the humanoid model -->
  <node pkg="robot_state_publisher" exec="robot_state_publisher" name="robot_state_publisher">
    <param name="use_sim_time" value="$(var use_sim_time)"/>
    <param name="robot_description" value="$(command 'xacro $(find-pkg-share digital_twin_gazebo)/models/simple_humanoid.urdf')"/>
  </node>

  <!-- Spawn the humanoid model in Gazebo -->
  <node pkg="gazebo_ros" exec="spawn_entity.py" 
        args="-entity simple_humanoid -topic robot_description -x 0 -y 0 -z 1.0">
    <param name="use_sim_time" value="$(var use_sim_time)"/>
  </node>
</launch>